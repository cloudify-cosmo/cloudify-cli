...
function download_wheels() {
    ...
    PATCH_URL="https://raw.githubusercontent.com/cloudify-cosmo/cloudify-cli/milestone-single-tar-tmp/packaging/omnibus/config/patches/cloudify-cli/cloudify_cli.patch"
    curl -sLO https://github.com/cloudify-cosmo/cloudify-cli/archive/milestone-single-tar-tmp.zip
    unzip -q -o milestone-single-tar-tmp.zip
    [[ -f milestone-single-tar-tmp.zip ]] && rm -f milestone-single-tar-tmp.zip
    curl -sL "${PATCH_URL}" -o cloudify-cli-milestone-single-tar-tmp/cloudify_cli.patch
    patch -p1 -d cloudify-cli-milestone-single-tar-tmp < cloudify-cli-milestone-single-tar-tmp/cloudify_cli.patch
    rm -f cloudify-cli-milestone-single-tar-tmp/cloudify_cli.patch
    zip -q -r cloudify-cli-milestone-single-tar-tmp.zip cloudify-cli-milestone-single-tar-tmp
    [[ $? -eq 0 ]] && rm -rf cloudify-cli-milestone-single-tar-tmp

    pip wheel --wheel-dir packaging/source/wheels --find-links packaging/source/wheels C:/Cygwin/home/Administrator/cloudify-cli-milestone-single-tar-tmp.zip \
