...
pip wheel git+https://github.com/cloudify-cosmo/cloudify-script-plugin@1.4.1 --wheel-dir=%{buildroot}/var/wheels/%{name} --find-links=%{buildroot}/var/wheels/%{name} --pre &&
...
curl https://raw.githubusercontent.com/cloudify-cosmo/cloudify-script-plugin/1.4.1/plugin.yaml -o %{buildroot}/opt/cfy/cloudify/plugins/script-plugin/plugin.yaml &&
...
import_resolver:
  parameters:
    ...
    - {'http://www.getcloudify.org/spec/script-plugin/1.4.1': 'file:/opt/cfy/cloudify/plugins/script-plugin'}
