...
def upload(blueprint_path, blueprint_id):
    ...
    lgr.info('Uploading blueprint {0} to management server {1}'
             .format(blueprint_path.name, management_ip))
    client = utils.get_rest_client(management_ip)
    blueprint = client.blueprints.upload(blueprint_path.name, blueprint_id)
    lgr.info("Uploaded blueprint, blueprint's id is: {0}"
             .format(blueprint.id))


def download(blueprint_id, output):
    management_ip = utils.get_management_server_ip()
    lgr.info(messages.DOWNLOADING_BLUEPRINT.format(blueprint_id))
    client = utils.get_rest_client(management_ip)
    target_file = client.blueprints.download(blueprint_id, output)
    lgr.info(messages.DOWNLOADING_BLUEPRINT_SUCCEEDED
...
def delete(blueprint_id):
    ...
    management_ip = utils.get_management_server_ip()
    lgr.info('Deleting blueprint {0} from management server {1}'
