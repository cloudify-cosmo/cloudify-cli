...
@cfy.options.common_options
def schedule():
    """Handle workflow execution scheduling [manager only]
    """
    pass


@schedule.command(name='create',
                  short_help='Schedule a workflow execution')
@cfy.argument('name')
...
def schedule_create(name,
                    ...
                    client,
                    logger):
    """Schedule an execution of a workflow on a given deployment
    """
    utils.explicit_tenant_name_message(tenant_name, logger)
...
@schedule.command(name='update',
                  short_help='Update a workflow execution schedule')
@cfy.argument('name')
@cfy.options.common_options
...
def schedule_update(name,
    ...
    """
    utils.explicit_tenant_name_message(tenant_name, logger)
    logger.info('Updating execution schedule %s...', name)
    ...
    logger.info('Execution schedule updated successfully')


@schedule.command(name='delete',
                  short_help='Delete a schedule for a workflow execution')
@cfy.argument('name')
@cfy.options.common_options
@cfy.options.tenant_name(required=False,
                         resource_name_for_help='execution schedule')
@cfy.assert_manager_active()
@cfy.pass_client()
@cfy.pass_logger
def schedule_delete(name, logger, client, tenant_name):
    ...
    utils.explicit_tenant_name_message(tenant_name, logger)
    logger.info('Deleting execution schedule %s...', name)
    client.execution_schedules.delete(name)
    logger.info('Execution schedule deleted successfully')


@schedule.command(name='list',
                  short_help='List execution schedules')
@cfy.options.sort_by()
@cfy.options.descending
@cfy.options.tenant_name_for_list(
    required=False, resource_name_for_help='execution schedule')
@cfy.options.all_tenants
@cfy.options.search
@cfy.options.pagination_offset
@cfy.options.pagination_size
...
def schedule_list(sort_by,
                  ...
                  logger,
                  client):
    """List all execution schedules on the manager
    """
    utils.explicit_tenant_name_message(tenant_name, logger)
    logger.info('Listing execution schedules...')
    ...
    print_data(SCHEDULE_TABLE_COLUMNS, schedules, 'Execution schedules:')
    total = schedules.metadata.pagination.total
    logger.info('Showing %s of %s execution schedules', len(schedules), total)


@schedule.command(name='get',
                  short_help='Retrieve execution schedule information')
@cfy.argument('name')
@click.option(
    '--preview',
    required=False,
    type=int,
    help="Preview N next dates for the execution to run."
)
@cfy.options.common_options
@cfy.options.tenant_name(required=False,
                         resource_name_for_help='execution schedule')
...
def schedule_get(name, preview, logger, client, tenant_name):
    """Retrieve information for a specific execution schedule

    `NAME` is the name of the execution schedule to get information on.
    """
    utils.explicit_tenant_name_message(tenant_name, logger)
    try:
        logger.info('Retrieving execution schedule %s', name)
        exec_schedule = client.execution_schedules.get(name)
    except exceptions.CloudifyClientError as e:
        if e.status_code != 404:
            raise
        raise CloudifyCliError('Execution schedule {0} not found'.format(name))

    columns = SCHEDULE_TABLE_COLUMNS
    extra_columns = ['rule', 'execution_arguments', 'all_next_occurrences']
    additional_data = {k.replace('_', ' '): v for k, v in exec_schedule.items()
                       if k not in SCHEDULE_TABLE_COLUMNS + extra_columns}
    if get_global_json_output():
        columns += additional_data.keys() + extra_columns
    print_single(columns, exec_schedule, 'Execution schedule:', max_width=50)

    if not get_global_json_output():
        print_details(exec_schedule['rule'], 'Scheduling rule:')
        print_details(exec_schedule['execution_arguments'],
                      'Execution arguments:')
        print_details(additional_data, 'Additional data:')

        if preview:
            next_occurrences = exec_schedule['all_next_occurrences']
