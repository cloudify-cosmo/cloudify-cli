...
def start_replace_certificates(input_path,
    ...
    _assert_cluster_is_healthy(client)  # Status needs to be OK
    ...
    _assert_cluster_is_healthy(client)  # Validate the status remained OK

    if main_config.new_cli_ca_cert():
        logger.info('Please replace the CLI CA cert in case you are not using '
                    'a load-balancer. You can do this using '
                    '`cfy profiles set -c <ca-cert-path>`')
    logger.info('\nSuccessfully replaced certificates')


def _assert_cluster_is_healthy(client):
    cluster_status = client.cluster_status.get_status()
    if cluster_status['status'] != 'OK':
        raise CloudifyCliError('The cluster status is FAILED')
