...
class LogsTest(CliCommandTest):
    def test_with_empty_config(self):
        ...
        ssh.profile = profile
        self.invoke('cfy logs download', ssh.SSH_ERR.format('user'))

    def test_with_no_key(self):
        profile = self.use_manager(ssh_key_path=None)
        ssh.profile = profile
        self.invoke('cfy logs download', ssh.SSH_ERR.format('key'))
    ...
    def test_with_no_port(self):
        ...
        ssh.profile = profile
        self.invoke('cfy logs download', ssh.SSH_ERR.format('port'))
    ...
    @patch('cloudify_cli.ssh.get_file_from_host')
    @patch('cloudify_cli.ssh.run_command_on_host')
    ...
    def test_download_with_no_output_path(self,
                                          _archive_logs_mock,
                                          run_command_on_host_mock,
                                          get_file_from_host_mock):
        profile = self.use_manager()
        ssh.profile = profile
        run_command_on_host_mock.side_effect = \
            self.__side_effect_mock_function__
        get_file_from_host_mock.side_effect = \
            self.__side_effect_mock_function__
