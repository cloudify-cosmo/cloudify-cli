...
class DeploymentUpdatesTest(CliCommandTest):
    ...
    def test_deployment_update_json_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment --json'
            .format(SAMPLE_BLUEPRINT_PATH))

    def test_deployment_update_include_logs_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment --include-logs'
            .format(SAMPLE_BLUEPRINT_PATH))

    def test_deployment_update_skip_install_flag(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment --skip-install'
            .format(SAMPLE_BLUEPRINT_PATH))

    def test_deployment_update_skip_uninstall_flag(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment --skip-uninstall'
            .format(SAMPLE_BLUEPRINT_PATH))

    def test_deployment_update_force_flag(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment --force'
            .format(SAMPLE_BLUEPRINT_PATH))

    def test_deployment_update_override_workflow_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} my_deployment -w override-wf'
            .format(SAMPLE_BLUEPRINT_PATH))
    ...
    def test_dep_update_archive_loc_and_bp_path_parameters_exclusion(self):
        self.invoke(
            'cfy deployments update -p {0} -n {1}/helloworld/'
    ...
    def test_deployment_update_blueprint_filename_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} -n my-blueprint.yaml '
            'my_deployment'.format(SAMPLE_ARCHIVE_PATH))

    def test_deployment_update_inputs_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} '
            '-i {1}/helloworld/inputs.yaml my_deployment'
            .format(SAMPLE_ARCHIVE_PATH, BLUEPRINTS_DIR))

    def test_deployment_update_multiple_inputs_parameter(self):
        self.invoke(
            'cfy deployments update -p {0} '
            '-i {1}/helloworld/inputs.yaml '
            '-i {1}/helloworld/inputs.yaml my_deployment'
            .format(SAMPLE_ARCHIVE_PATH, BLUEPRINTS_DIR))

    def test_deployment_update_no_deployment_id_parameter(self):
        self.invoke(
            'cfy deployments update -p {0}'.format(SAMPLE_ARCHIVE_PATH),
            should_fail=True,
